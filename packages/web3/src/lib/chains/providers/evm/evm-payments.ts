...
async function handleCustomTokenApproval(
    ...
    try {
        ...
        const approvalAmount = await customTokenContract['allowance'](signerAddress, contractAddress);
...
export const droplinked_payment = async function(
    ...
    if (!data.tbdValues) {
        throw new InvalidParametersException(`Invalid tbdValues: ${data.tbdValues}`);
    }

    if (!data.tbdReceivers) {
        throw new InvalidParametersException(`Invalid tbdReceivers: ${data.tbdReceivers}`);
    }

    if (!data.memo) {
        throw new InvalidParametersException(`Memo is required for payment, current memo: ${data.memo}`);
    }
